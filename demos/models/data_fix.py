...
def validate_persons_table(persons, households):
    ...
    if len(no_head_hhs) > 0:
        if raise_errors:
            raise ValueError(f"{len(no_head_hhs)} households with no head in input data")
        if fix_inconsistent:
            logger.warning(f"{len(no_head_hhs)} households with no head. {(no_head_hh_sizes == 1).sum()} were single-person households, the person was made head. The other {(no_head_hh_sizes != 1).sum()} were dropped.")
    ...
    if len(multi_partner_hh) > 0:
        if raise_errors:
            raise ValueError(f"{len(multi_partner_hh)} households with multiple partners")
        if fix_inconsistent:
            logger.warning(f"{len(multi_partner_hh)} households with multiple partners ({len(persons.local.loc[persons.household_id.isin(multi_partner_hh)])} total people) were dropped")
    ...
    if married_head_no_spouse.sum() > 0:
        if raise_errors:
            raise ValueError(f"{married_head_no_spouse.sum()} heads have MAR == 1 but no spouse in house.")
        if fix_inconsistent:
            logger.warning(f"{married_head_no_spouse.sum()} heads have MAR == 1 but no spouse in house. Changing to MAR = 0")
            persons.local.loc[~married_hh_idx & head_or_spouse_idx & (persons.MAR == 1), "MAR"] = 0

    if len(hhs_with_wrong_MAR) > 0:
        if raise_errors:
            raise ValueError(f"{len(hhs_with_wrong_MAR)} households have married people but the number of MAR == 1 is different than 2.")
        if fix_inconsistent:
            logger.warning(f"{len(hhs_with_wrong_MAR)} households have married people but the number of MAR == 1 is different than 2. Spouses were flagged with MAR = 1")
    ...
    if cohabitating_people_married_idx.sum():
        if raise_errors:
            raise ValueError(f"{cohabitating_people_married_idx.sum()} people cohabitating (relate == 0 or 13), are flagged as married (MAR == 1).")
        if fix_inconsistent:
            logger.warning(f"{cohabitating_people_married_idx.sum()} people cohabitating (relate == 0 or 13), are flagged as married (MAR == 1). Changing to 0")
