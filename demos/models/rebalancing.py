...
def household_rebalancing(
    ...
    marital_rebalanced.local = pd.concat(
        [
            marital_rebalanced.local,
            pd.DataFrame(
                [
                    [
                        year,
                        (orca.get_table("persons").local.MAR == 1).sum(),
                        (orca.get_table("persons").local.MAR == 3).sum(),
                    ]
                ],
    ...
    assert (
        GEOID_COL in control_table_wrapped.local_columns
    ), f"{GEOID_COL} must be in {CONTROL_TABLE}"
    assert (
        CONTROL_COL in control_table_wrapped.local_columns
    ), f"{CONTROL_COL} must be in {CONTROL_TABLE}"
    assert (
        control_table_wrapped.index.name == "year"
    ), f"The index of {CONTROL_TABLE} must be 'year'"
    assert (
        len(control_table_wrapped.local_columns) == 3
    ), f"{CONTROL_TABLE} needs to have exactly 3 columns: {GEOID_COL}, {CONTROL_COL} and the value column"
    assert (
        persons.household_id.nunique() == households.index.nunique()
    ), f"`persons` and `households` tables do not have coherent sizes. {persons.household_id.nunique()} vs. {households.index.nunique()}"
    ...
    value_column = [
        c
        for c in control_table_wrapped.local_columns
        if c not in [GEOID_COL, CONTROL_COL]
    ][0]
    index_df = households.to_frame([GEOID_COL, CONTROL_COL]).sort_values(
        [GEOID_COL, CONTROL_COL]
    )
    indices = index_df.groupby([GEOID_COL, CONTROL_COL]).indices
    current_count = index_df.groupby([GEOID_COL, CONTROL_COL]).size()
    hh_difference = (
        control_table_wrapped.local.loc[year]
        .astype({GEOID_COL: "str", CONTROL_COL: "str"})
        .set_index([GEOID_COL, CONTROL_COL])[value_column]
        .loc[current_count.index]
        - current_count
    )
    ...
    for (geo_id, hh_size), adjustment in hh_difference.items():
        ...
        selected_hh = np.random.choice(
            valid_indices,
            size=abs(adjustment),
            replace=(adjustment > 0) and (abs(adjustment) > len(valid_indices)),
        ).tolist()
        if adjustment < 0:
    ...
    new_hh_ids = get_new_households(
        len(to_duplicate_hh)
    )  # Remeber that this creates household rows
    new_hh_rows = households.local.loc[to_duplicate_hh].copy()
    new_hh_rows.index = new_hh_ids

    hh_mapping = pd.DataFrame({"orig_hh": to_duplicate_hh, "new_hh": new_hh_ids})
    new_person_rows = persons.local[persons.household_id.isin(to_duplicate_hh)].copy()
    new_person_rows = new_person_rows.merge(
        hh_mapping, left_on="household_id", right_on="orig_hh"
    )
    ...
    to_remove_hh.sort()
    rebalanced_households.local = pd.concat(
        [rebalanced_households.local, households.local.loc[to_remove_hh]]
    )
    rebalanced_persons.local = pd.concat(
        [
            rebalanced_persons.local,
            persons.local[persons.household_id.isin(to_remove_hh)],
        ]
    )
