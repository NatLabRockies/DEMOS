...
def update_married_households_random(
    ...
    newmarried["hh_group"] = np.arange(len(newmarried)) % (
        len(newmarried) // 2
    )  # [0, 1, 2, ..., n_weddings -1, 0, 1, ..., n_weddings - 1]
    ...
    newcohab["hh_group"] = (
        (np.arange(len(newcohab)) % (len(newcohab) // 2))
        + newmarried["hh_group"].max()
        + 1
    )
    ...
    swap_selector = (
        np.arange(len_all_df)
        + (1 - (np.arange(len_all_df) % 2))
        + ((-1) * (np.arange(len_all_df) % 2))
    )
    ...
    all_df["partner_house_id"] = all_df.loc[swap_index][
        "household_id"
    ].values  # NOTE: The `.values` part is very important
    ...
    first_index = (
        all_df["first"] == 1
    )  # NOTE: `first` is a method of pd.DataFrame so we need to use this notation
    all_df["new_hh_id"] = np.nan

    ### If first is head of household, the new partner moves in
    first_and_head_index = head_index & first_index
    first_and_head_partners_index = all_df.loc[
        first_and_head_index, "partner_id"
    ].values
    all_df.loc[first_and_head_index, "new_hh_id"] = all_df.loc[
        first_and_head_index, "household_id"
    ].values
    all_df.loc[first_and_head_partners_index, "new_hh_id"] = all_df.loc[
        first_and_head_index, "household_id"
    ].values
    ...
    first_and_not_head_partners_index = all_df.loc[
        first_and_not_head_index, "partner_id"
    ].values
    all_df.loc[first_and_not_head_index, "new_hh_id"] = all_df.loc[
        first_and_not_head_partners_index, "household_id"
    ].values
    all_df.loc[first_and_not_head_partners_index, "new_hh_id"] = all_df.loc[
        first_and_not_head_index, "new_hh_id"
    ].values

    ### If neither is head, form a new household
    neither_head_index = (all_df.relate != 0) & (all_df.partner_relate != 0)
    neither_head_not_first_index = all_df.loc[
        first_index & neither_head_index
    ].partner_id.values
    ...
    assert (
        all_df.new_hh_id.isnull().sum() == 0
    ), "Some people were not assigned a household"
    ...
    new_household_heads = (
        persons.local[persons["household_id"].isin(head_left_households)]
...
def update_divorce(persons, households, divorce_list, get_new_households):
    ...
    person_in_divorced_household_index = persons["household_id"].isin(
        divorced_household_ids
    )
    head_and_spose_index = ((persons["relate"] == 0) | (persons["relate"] == 1)) & (
        persons["MAR"] == 1
    )

    people_divorcing_groupby = (
        persons.local.loc[person_in_divorced_household_index & head_and_spose_index]
        .sort_index()
        .groupby("household_id")
    )
    assert (
        people_divorcing_groupby.size() != 2
    ).sum() == 0, (
        "Some divorcing households have more than 2 people eligible for divorce"
    )
    ...
    persons.local.loc[
        staying_household_index & staying_member_id_filter, "member_id"
    ] = (
        persons.local.loc[
            staying_household_index & staying_member_id_filter, "member_id"
        ]
    ...
    persons.local.loc[
        staying_household_index & ~staying_member_id_filter, "member_id"
    ] = persons.local.loc[staying_household_index & ~staying_member_id_filter, "relate"]
