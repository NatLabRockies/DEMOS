...
data_name = 'scag_rtp24/synpop_2019'
...
observed_marrital_data_name = "data/scag_rtp24/calibration/marrital_status_over_time_obs_SCAG_19_23_ACS5.csv"
...
for table_name in synpop_tables:
    step_name = "load_" + table_name
    table_path = synpop_path + table_name + ".csv"
    if table_name == "households":
        table_index_cols = "household_id"
    elif table_name == "persons":
        table_index_cols = "person_id"       
    LoadTable(
        table=table_name,
        source_type="csv",
        path=table_path,
        csv_index_cols=table_index_cols,
        extra_settings={"dtype": {"block_id": object, "TAZ": object, "lcm_county_id": object}},
        name=step_name,
    ).run()
...
persons['education_group'] = pd.cut(persons['eduatt'], bins=education_intervals, labels=education_labels, include_lowest=True).astype(str)
