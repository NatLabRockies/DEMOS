...
def initialize(path="configs"):
    ...
    if len(steps) == 0:
        logger.warning(
            "No files from ModelManager 0.1.dev8 or later found in path '{}'".format(
                path
            )
        )
...
def load_supplemental_object(step_name, name, content_type, required=True):
    ...
    if content_type == "pickle":
        with open(
            os.path.join(_disk_store, step_name + "-" + name + ".pkl"), "rb"
        ) as f:
...
def save_step_to_disk(step):
    ...
    print("Saving '{}.yaml': {}".format(name, os.path.join(os.getcwd(), _disk_store)))
